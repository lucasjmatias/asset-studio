...
7. Bones use a unified smart manipulator. Dragging the middle moves any bone and its descendants without moving its parent. A child bone has a large parent-linked joint and a smaller free endpoint: dragging the free endpoint rotates around the large joint, while the endpoint ring resizes along the bone axis.
...
15. Export the active frame, or export every named pose as a single horizontal PNG tileset. Rest is never included automatically; create a named rest-like pose when you want one exported. When Pixel AI is enabled, the whole sheet learns one shared palette for frame-to-frame color consistency.
